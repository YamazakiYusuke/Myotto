...
Sentence.make_sentences_from_book(Book.last.id, content )
...
translations = [["Alice was beginning to get very tired of sitting by her sister on the bank, and of having nothing to do: once or twice she had peeped into the book her sister was reading, but it had no pictures or conversations in it, “and what is the use of a book,” thought Alice “without pictures or conversations?”",
  ...
  ["妖精たちはアリスの左右に分かれ、コーラスの声は優しく遠のいてゆく。",
    "場所は変わり、不思議の国のどこかの庭。",
    "わきの方に大きなキノコがあり、その上で芋虫が煙管をふかしている。",
    "アリスは目を開けると、何が何だか分からないというふうに、あたりをうろちょろする。",
    "そこへ白いウサギがあわてたていで、前を横切ってゆく。",
    "白ウサギは通りすぎ、その際に白手袋と扇を落としていく。"],
  ["Никаких других деталей на Смоленской площади не привели.",
  "Российский посол в Вашингтоне Анатолий Антонов с конца марта находится в Москве, куда его пригласили для консультаций.",
  "Это произошло после того, как Джо Байден в интервью телеканалу ABC News заявил, что российский лидер заплатит за вмешательство в выборы.",
  "При этом глава Белого дома утвердительно ответил на вопрос, считает ли он Владимира Путина убийцей.",
  "Вскоре Путин и Байден пообщались по телефону, американский лидер предложил встретиться на нейтральной территории, однако сразу же после этого разговора США ввели новые санкции против России. ",
  "В МИД отметили, что подобный курс не отвечает интересам народов двух государств."]]

20.times do |n|
  user_ids.each do |t|
    sentence_id = sentence_ids.sample
    book = Sentence.find(sentence_id).book
    locale = book.book_locale_statuses[0].locale_id + 1
    if locale == 4
      locale -= 4
    else
      locale -= 1
    ...
    Translation.create!(
      content: translations[locale].sample,
      user_id: t,
      sentence_id: sentence_id,
      book_locale: book.book_locale_statuses[0].locale.name,
      user_locale: User.find(t).user_locale_statuses.find_by(is_wanted: true).locale.name,
    )
  end
end
